...
class ParticipantController {
  async addParticipant(req, res) {
    try {
      ...
      if (!roomId || !userId) {
        return res
          .status(400)
          .json({ error: "ID da sala e ID do usuário são obrigatórios" });
      }
      ...
      if (result.exists) {
        return res
          .status(200)
          .json({ message: "Usuário já é participante desta sala" });
      }
  ...
  async removeParticipant(req, res) {
    try {
      ...
      const removed = await ParticipantRepository.removeParticipant(
        roomId,
        userId
      );
      ...
      return res
        .status(200)
        .json({ message: "Participante removido com sucesso" });
    } catch (error) {
